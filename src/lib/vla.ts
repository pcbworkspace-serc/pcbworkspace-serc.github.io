...
function simulatePlan(instruction: string, boardState: BoardStateItem[]): VLAPlan {
  ...
  // Decide a target XY based on keywords in the instruction
  ...
  if (corner) {
    ...
    // Try to extract explicit coordinates like "at 25, 30" or "25mm 30mm"
    const xy = lower.match(/(\d+(?:\.\d+)?)\s*(?:mm)?[\s,]+(\d+(?:\.\d+)?)\s*(?:mm)?/);
    if (xy) target = { x: parseFloat(xy[1]), y: parseFloat(xy[2]) };
  }

  // Default pick location (a stand-in for the parts feeder)
  const pickup = { x: 50, y: 8, z: safeZ };

  // Always home first unless the user said otherwise
  ...
  const wantsScan  = /\b(scan|inspect|look at)\b/.test(lower);
  ...
  // End trip home if we did anything substantial
  if (!skipHome && actions.length > 1 && actions[actions.length - 1].action !== "home") {
    actions.push({ action: "home" });
  }

  const interpretation = actions.length === 0
    ? `Demo Mode couldn't parse "${instruction}". Try keywords: home, move, pick, place, scan.`
    : `Demo Mode plan (no LLM): ${describeActions(actions)}.`;
  ...
  return {
    ...
    warnings: actions.length === 0
      ? ["Demo Mode uses keyword matching. With a real Anthropic API key in Flask, VLA can handle any instruction."]
      : ["Plan generated by Demo Mode stub — not as smart as Claude. Run a local Flask with ANTHROPIC_API_KEY for real VLA."],
    model: "demo-stub",
    tokens_in: 0,
    tokens_out: 0,
    elapsed_ms: 200,
